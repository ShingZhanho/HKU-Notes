---
description: Syntax reference for metadata.json schema version 1 (v1) used in the HKU Notes project.
---

!!! warning "Historical documentation — unsupported"

    Schema v1 is no longer accepted by the build tools. Use [schema v3](v3.md).
...
!!! danger "Schema v1 Has Been Phased Out"
    ...
    Schema v1 is **no longer supported**. All existing build targets have been migrated to [schema v2](v2.md).

    **Pull requests introducing build targets with v1 schema will not be merged** until the schema is updated to v2.

    This page is kept for **historical reference only**. Do not use v1 for any new or existing build targets.
...
of the static site page for the target. It should use the Markdown syntax supported by
Zensical.
- **Type**: `string`
...
- **Accepted Values**: Any valid icon name from the [Material Design Icons set](https://squidfunk.github.io/mkdocs-material/reference/icons-emojis/),
