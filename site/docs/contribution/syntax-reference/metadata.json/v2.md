...
!!! warning "Historical documentation — unsupported"
    ...
    Schema v2 is no longer accepted by the build tools. Use [schema v3](v3.md).
...
!!! success "Required Schema Version"

    Schema v2 is the **only supported schema** for all build targets. All existing targets have been migrated to v2.

    **Pull requests using v1 schema will not be merged** until updated to v2.
