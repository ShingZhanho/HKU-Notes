...
def set_ST():
    ...
    \begin{solution}
        ...
        \textbf{Explanation:} After the first four lines, the registers are initialized as follows:
        \begin{itemize}
            \item \texttt{R4} = 0 (loaded from memory address \texttt{P0 - 0x3C})
            \item \texttt{R1} = 1 (loaded from memory address \texttt{P1 - 0x40})
            \item \texttt{R2} = 1 (copied from \texttt{R1}, this is a constant as no operations use it as destination)
            \item \texttt{R3} = 10 (loaded from memory address \texttt{P2 - 0x44}, also a constant)
        \end{itemize}
        In every iteration of the loop labeled \texttt{L:}, the following operations occur:
        \begin{itemize}
            \item \texttt{R4} is incremented by the value in \texttt{R1}
            \item \texttt{R1} is incremented by the constant 1 (from \texttt{R2})
            \item \texttt{R5} is set to be \texttt{R3 (10) - R1}
            \item If the previous operation results in non-zero, i.e., \texttt{R1} $\neq$ 10, branch back to \texttt{L:}
        \end{itemize}
        When \texttt{R1} reaches 10, \texttt{R4} will have accumulated the sum of integers from 1 to 9.
        Finally, the value in \texttt{R4} is stored back to memory address \texttt{P - 0x48}, and the program halts.
    \end{solution}

    \section*{Part II: Hand Assemble}

    Translate the following program into hexadecimal form, and put it in a file named prog2
    with the same format as the file prog. Run the simulator by
    \vspace*{-0.2cm}\begin{verbatim}
    ...
    A working simulator (executable \texttt{.pyc} file only, without source code) is given to you, so
    that you can complete this part using this program, if your simulator in Part I is not
    working.

    \begin{solution}
        The program is hand assembled as follows:
        \begin{verbatim}
            0600ff04
            0000003c
            0600ff01
            ...
            00000000
        \end{verbatim}
        The purpose of this program is to compute the sum of $5+5+5+5=20$.
        The final result stored in memory address \texttt{P} is \texttt{0x00000014}.
    \end{solution}
