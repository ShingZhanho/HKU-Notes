...
\begin{questions}
    ...
    \begin{parts}
        ...
        \begin{solution}
            Let $L$ be the event that the mail is labelled as spam, $\overline{L}$ be the event that the mail
            is labelled as non-spam, $O$ be the event that the model outputs spam for the mail, and 
            $\overline{O}$ be the event that the model outputs non-spam for the mail.

            From the given information, we have
            \begin{align*}
                P(L) = 0.1, &\quad P(\overline{L}) = 0.9, \\
                P(O\mid L) = 0.8, &\quad P(\overline{O}\mid \overline{L}) = 0.9.
            \end{align*}

            We need to find $P(L\cap O)+P(\overline{L}\cap \overline{O})$.

            By conditional probabilities, we have $P(O\mid L) = \frac{P(L\cap O)}{P(L)} \Leftrightarrow P(L\cap O) = P(O\mid L) \cdot P(L) = 0.8 \cdot 0.1 = 0.08$.
            Similarly, we have $P(\overline{O}\cap\overline{L}) = P(\overline{O}\mid\overline{L})\cdot P(\overline{L}) = 0.9 \cdot 0.9 = 0.81$.

            Therefore, the required probability is $P(\text{output matches label})=0.08 + 0.81 = \boxed{0.89}$.
      \end{solution}
        ...
        \begin{solution}
            We need to find $P(L\mid O)=\frac{P(L\cap O)}{P(O)}$.

            By the law of total probability, we have $P(O)=P(O\mid L)P(L)+P(O\mid\overline{L})P(\overline{L})
            =0.8\cdot0.1+\left[1-P(\overline{O}\mid\overline{L})\right]\cdot0.9
            =0.08+(1-0.9)\cdot0.9=0.08+0.1\cdot0.9=0.08+0.09=0.17$.

            Therefore, $P(L\mid O)=0.08/0.17=\boxed{\frac{8}{17}}$.
        \end{solution}
        ...
        \begin{solution}
            Let $C$ be the event that the label is correct, and $\overline{C}$ be the event that the label is flipped.
            We have $P(C)=0.95$ and $P(\overline{C})=0.05$.

            When the label is correct ($C$), from part (\ref{part@8@1}), we know the probability that the model's
            output matches the label is 0.89. Since the label is correct, this is also the probability that the
            model categorizes the email correctly.

            When the label is flipped ($\overline{C}$), the model's output matches the (incorrect) label with
            the same probability 0.89. Therefore, the model categorizes the email correctly (i.e., matches the
            true label) with probability $1-0.89=0.11$.

            By the law of total probability, the probability that the model categorizes a random email correctly is
            \begin{align*}
                P(\text{correct}) &= P(\text{correct}\mid C)P(C) + P(\text{correct}\mid\overline{C})P(\overline{C})\\
                &= 0.89\cdot0.95 + 0.11\cdot0.05\\
                &= 0.8455 + 0.0055\\
                &= \boxed{0.851}.
            \end{align*}
        \end{solution}
        ...
        \begin{solution}
            Note that when there is only 1 passenger, they will always sit in their assigned seat.
            ...
            Thus, we have $\boxed{P(E^n_n)=\begin{cases}
